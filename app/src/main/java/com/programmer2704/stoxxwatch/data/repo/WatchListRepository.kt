...
import com.programmer2704.stoxxwatch.data.models.WatchListItem_
...
class WatchListRepository @Inject constructor(database: DatabaseManager){
    ...
    private fun find(company: Company) : WatchListItem? {
        return box.query()
            .equal(com.programmer2704.stoxxwatch.data.models.WatchListItem_.name, company.name)
            .and()
            .equal(com.programmer2704.stoxxwatch.data.models.WatchListItem_.symbol, company.symbol)
            .build()
            .findFirst()
    }
    ...
    fun delete(item: WatchListItem) {
        box.query()
            .equal(com.programmer2704.stoxxwatch.data.models.WatchListItem_.name, item.name)
            .and()
            .equal(com.programmer2704.stoxxwatch.data.models.WatchListItem_.symbol, item.symbol)
            .build()
            .remove()
    }
